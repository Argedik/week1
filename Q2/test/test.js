...
describe("HelloWorld", function () {
  ...
  it("Should return true for correct proof", async function () {
    ...
    //We generating proof parameters for the input
    ...
    //We show the input parameters(publicSignals[0]) of our circuit
    console.log("1x2 =", publicSignals[0]);

    //We convert array elements "editedPublicSignals" and "editedProof" from string to BigInt
    const editedPublicSignals = unstringifyBigInts(publicSignals);
    const editedProof = unstringifyBigInts(proof);

    //We Generate copy-pasteable input parameters to call the verify method in the Solidity contract
    const calldata = await groth16.exportSolidityCallData(
      editedProof,
      editedPublicSignals
    );

    //We make nested arrays into a single array
    const argv = calldata
      .replace(/["[\]\s]/g, "")
      .split(",")
      .map((x) => BigInt(x).toString());

    //We reconstruct parameters a, b, c for validation
    ...
    //We Verify the ZK proof with hashed parameters (a, b, c) and public output 2 (Input)
...
describe("Multiplier3 with PLONK", function () {
  ...
  it("Should return true for correct proof", async function () {
    ...
    // console.log(calldata, "calldata hakkında");
    // console.log(argv, "argv hakkında");

    const proofInput = argv[0];
    const publicSignalsInput = [argv[1]];

    expect(await verifier.verifyProof(proofInput, publicSignalsInput)).to.be
      .true;
  });
  it("Should return false for invalid proof", async function () {
    //[assignment] insert your script here
